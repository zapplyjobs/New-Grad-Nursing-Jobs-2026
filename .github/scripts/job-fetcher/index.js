 ...
 *
 * Fetches nursing/healthcare jobs from centralized aggregator and generates README.
 * Replaced JSearch direct fetch with aggregator consumer (2026-02-18).
 *
 * Note: Aggregator tags nursing/healthcare jobs under 'general' domain.
 * A dedicated nursing domain will be added when aggregator tagging improves.
 *
 * Pipeline:
 * 1. Fetch general jobs from aggregator (jobs-data-2026/all_jobs.json)
...
async function main() {
  try {
    console.log('🚀 Starting Nursing/Healthcare jobs fetching system...');
    console.log('═'.repeat(50));

    // Fetch general jobs from aggregator (nursing falls under 'general' domain)
    const consumer = createAggregatorConsumer({
      filters: { domains: ['general'] },
      verbose: true
    });
    ...
    if (jobs.length === 0) {
      console.log('⚠️  No nursing/general jobs fetched from aggregator');
